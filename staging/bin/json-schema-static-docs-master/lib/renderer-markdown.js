...
const propertyRow = (name, property, parentKey, isRequired) => {
  let href = "";
...
const propertiesTable = (schema, parentKey) => {
  ...
  for (const key in properties) {
    ...
    if (
      ...
      const isRequired=schema.required && schema.required.indexOf(key)>=0;
      html += propertyRow(key, property, parentKey,isRequired);
    }
  }
